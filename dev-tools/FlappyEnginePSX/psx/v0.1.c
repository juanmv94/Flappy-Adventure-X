...
static void read_pad(SVECTOR* ang, VECTOR* pos) {
	...
	if(pad & PADLup) {pos->vz-=rcos(ang->vy)>>8; pos->vx+=rsin(ang->vy)>>8;}
	if(pad & PADLdown) {pos->vz+=rcos(ang->vy)>>8; pos->vx-=rsin(ang->vy)>>8;}
	if(pad & PADLleft) {pos->vz+=rsin(ang->vy)>>8; pos->vx+=rcos(ang->vy)>>8;}
	if(pad & PADLright) {pos->vz-=rsin(ang->vy)>>8; pos->vx-=rcos(ang->vy)>>8;}
	if(pad & PADL2) pos->vy-=8;
	if(pad & PADR2) pos->vy+=8;
}
...
int main() {
	...
	SetGeomScreen(512);			//distance from viewpoint to screen
	...
	while(1) {
		...
		for (i=0;i<polyCount;i++) {
			...
			if (flags&0x01) {	//quad
				...
				if ((TransRotPers(&x[0],(long*)&f4.x0,&dmy,&flg) > 16) + (TransRotPers(&x[1],(long*)&f4.x1,&dmy,&flg) > 16) +
				...
				if ((TransRotPers(&x[0],(long*)&f3.x0,&dmy,&flg) > 16) + (TransRotPers(&x[1],(long*)&f3.x1,&dmy,&flg) > 16) +
					(TransRotPers(&x[2],(long*)&f3.x2,&dmy,&flg) > 16))
					if (flags&0x02 || NormalClip(*(long*)&f3.x0,*(long*)&f3.x1,*(long*)&f3.x2)>0) DrawPrim(&f3);
				p+=21;
			}
		}
		FntPrint("Flappy Engine PSX v0.1 - Juanmv94\n\n");
